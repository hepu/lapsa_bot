...
define('BOT_TOKEN', '242926534:AAHdX83PAqVb_Tc3oND-BjR4L6K6CFYfO7Y');
...
function processMessage($message) {
  ...
  if (isset($message['text'])) {
	...
	if($random_action >= 1 && $random_action <= 200){
		//Translajan
	...
	}else if($random_action > 201 && $random_action <= 240){
		//Transla iiiii
	...
	}else if($random_action > 401 && $random_action <= 500){
		if(strrpos($message['text'], '?')){
			$random_action_2 = rand(1,100);
			if($random_action_2 >= 1 && $random_action_2 <= 50){
				apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => 'para ke kieres saver eso. Jaja Saludoz.'));
			}else{
				apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => 'Yo ke se. Ni ke fuera 100tifiko'));
			}
		}
	}else if($random_action > 501 && $random_action <= 525){
		apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => 'Ichii'));
	}else if($random_action > 601 && $random_action <= 800){
		if(strrpos(strtolower($message['text']), 'firme') || strrpos(strtolower($message['text']), 'firme') === 0){
			$random_action_2 = rand(1,100);
			if($random_action_2 >= 1 && $random_action_2 <= 40){
				apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => 'Firme como carrito e peto en bajada.'));
			}else if($random_action_2 > 41 && $random_action_2 <= 70){
				apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => 'Firme como el flan'));
			}else if($random_action_2 > 41 && $random_action_2 <= 70){
				apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => 'Firme como la gelatina'));
			}
		}else if(strrpos(strtolower($message['text']), 'elegante') || strrpos(strtolower($message['text']), 'elegante') === 0){
			$random_action_2 = rand(1,100);
			if($random_action_2 >= 1 && $random_action_2 <= 50){
				apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => 'Elegante como el elefante.'));
			}else{
				apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => 'Elegante como el pegante.'));
			}
		}else if(strrpos(strtolower($message['text']), 'chirri') || strrpos(strtolower($message['text']), 'chirri') === 0){
			$random_action_2 = rand(1,100);
			if($random_action_2 >= 1 && $random_action_2 <= 50){
				apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => 'Chirri como '.$usuario));
			}else{
				apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => 'Chirrete.'));
			}
			break;
		}else if(strrpos(strtolower($message['text']), 'mkit') || strrpos(strtolower($message['text']), 'mkit') === 0){
			$random_action_2 = rand(1,100);
			if($random_action_2 >= 1 && $random_action_2 <= 50){
				apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => $usuario.' serás mkite'));
			}else{
				apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => 'mkite como '.$usuario));
			}
		}
	}
